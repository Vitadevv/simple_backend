...
def game_loop(player: Player, language: str = None):  # type: ignore
  ...
  while player.is_alive():
    ...
    slow_print(cfg["hp_coins_now"].format(int(player.hp), player.coins), delay=0.02, hold=0.5)
    ...
    for idx, path in enumerate(paths_gen, 1):
      ...
      risk_display = full_config.get("risk_labels", {}).get(risk_key, risk_key.replace("_", " ").title())
      slow_print(
        cfg["choose_your_path_message"].format(idx, risk_display, path["hint_text"]),
        delay=0.01,
        hold=0.2
      )
    ...
    log.info(f"Turn {turn_count}: {player.name} chose path with risk '{path_choice['risk']}' and hint '{path_choice['hint_id']}'")
    ...
    log.info(f"Turn {turn_count}: Event '{event_type}' occurred with value '{event_value}'. Player HP: {hp_before} → {player.hp}, Floor: {floor_before} → {player.floor}")
    ...
    if event_type == "enemy" and "report" in event_result:
      ...
      if report.get("winner") == player.name:
        slow_print(cfg["victory_message"].format(enemy_name), delay=0.03, hold=0.5)
      else:
        slow_print(cfg["defeat_message"].format(enemy_name), delay=0.03, hold=0.5)
    ...
    if event_type in ("floor_up", "floor_down") or player.floor != floor_before:
      ...
      if milestone_reward > 0:
        ...
        log.info(f"{player.name} reached milestone floor {player.floor} and gained {milestone_reward} coins")
    ...
    if not player.is_alive():
      slow_print(cfg.get("player_death", "You have fallen..."), delay=0.05, hold=1.0)
      log.info(f"{player.name} died on floor {player.floor} with {int(player.hp)} HP")
      break
  ...
  if player.best_run:
    ...
    slow_print(cfg["best_run_message"].format(player.best_run[0], player.best_run[1]), delay=0.03, hold=1.0)
  
  log.info(f"{player.name}'s run ended after {turn_count} turns. Best floor: {player.best_run[0] if player.best_run else 0}")
