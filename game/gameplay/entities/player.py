...
class Player(Entity):
  def __init__(self, name: str, hp: int | float, attack: int | float, speed: int | float):  
    ...
    log.info(f"{self.name} has been created with the following stats:\n{self.hp}\n{self.attack}\n{self.speed}\nEquipment: {self.equipment}")
  ...
  def remove_item(self, item: Item, quantity: int = 1) -> None:
    ...
    if quantity > current: 
      self.flags += 1
      log.warning(f"{self.name} attempted to remove {item.name} in quantity of {quantity}. Flagged")
      raise EquipmentError(f"Not enough {item.name} to remove. You currently have {current} of {item.name}, {quantity} requested.")
      
    new_quantity = current - quantity 
    if new_quantity == 0:
      del self.inventory[item]
      log.info(f"{item.name} deleted from {self.name}'s inventory because quantity reached 0")
    else:
  ...
  def end_timer(self):
    ...
    if self.start_time is None:
      self.flags += 1
      log.warning(f"{self.name} tried to end the timer without starting it. Flagged")
    ...
    if self.best_run is None:
      ...
      log.info(f"{self.name} has made their first run ever.\nFloor: {self.floor}\nDuration: {duration}")
      return
      
    #higher floor
    if self.best_run[0] < self.floor:
      self.best_run = (self.floor, duration)
      log.info(f"{self.name} has made it to a higher floor.\nFloor: {self.floor}\nDuration: {duration}")
      return
      
    #same floor but quicker
    if self.best_run[0] == self.floor and self.best_run[1] > duration:
      self.best_run = (self.floor, duration)
      log.info(f"{self.name} has made a new record on the same floor.\nFloor: {self.floor}\nDuration: {duration}")
      return
  ...
  def end_run(self):
    ...
    if self.start_time is None:
      self.flags += 1
      log.warning(f"{self.name} tried to end run without starting a timer. Flagged")
