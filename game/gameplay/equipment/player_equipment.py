...
def _validate_slot(player: "Player", slot: str) -> None:
...
def _validate_item_available(player: "Player", item: Item) -> None:
  if not isinstance(item, Item):
    raise TypeError("item must be an Item instance")
  if player.inventory.get(item, 0) < 1:
    log.warning(f"{player.name} couldn't equip {item.name} as it wasn't in their inventory")
...
def equip_item(player: "Player", slot: str, item: Item) -> dict:
